...
(function ($) {
    ...
    $(document).on('click', '.comment-navigation a', function (e) {
        ...
        $.ajax({
            ...
            url: this.href,
            beforeSend: function () {
                $('#comments .comment-list, #comments .comment-navigation').animate({ opacity: '0.1' });
                $('#comments .comment-navigation').before('<div class="loading"></div>');
            },
            dataType: "html",
            success: function (out) {
                let result = $(out).find('.comment-list').html();
                let nextlink = $(out).find('.comment-navigation').html();
                $('#comments .loading').remove();
                $('#comments .comment-list').html(result).animate({ opacity: '1' });
                $('#comments .comment-navigation').html(nextlink).animate({ opacity: '1' });
